...
namespace 企业管理.ModuleClass
{
    class MyModule
    {
        #region  公共变量
        DataClass.MyMeans MyDataClass = new 企业管理.DataClass.MyMeans();   //声明MyMeans类的一个对象，以调用其方法
		...
		public void Show_Form(string FrmName, int n)
        {
            if (n == 1)
            {
                ...
                {
                    PerForm.F_ManFile FrmManFile = new 企业管理.PerForm.F_ManFile();
                ...
                if (FrmName == "员工资料查询")
                {
                    PerForm.F_Find FrmFind = new 企业管理.PerForm.F_Find();
                    FrmFind.Text = "员工资料查询";
                    FrmFind.ShowDialog();
                    FrmFind.Dispose();
                }
                if (FrmName == "员工资料统计")
                {
                    PerForm.F_Stat FrmStat = new 企业管理.PerForm.F_Stat();
                    FrmStat.Text = "员工资料统计";
                    FrmStat.ShowDialog();
                    FrmStat.Dispose();
                }
                ...
                if (FrmName == "日志")
                {
                    PerForm.F_WordPad FrmWordPad = new 企业管理.PerForm.F_WordPad();
                    FrmWordPad.Text = "日志";
                ...
                if (FrmName == "通讯录")
                {
                    PerForm.F_AddressList FrmAddressList = new 企业管理.PerForm.F_AddressList();
                ...
                if (FrmName == "备份/还原数据库")
                {
                    PerForm.F_HaveBack FrmHaveBack = new 企业管理.PerForm.F_HaveBack();
                ...
                if (FrmName == "清空数据库")
                {
                    PerForm.F_ClearData FrmClearData = new 企业管理.PerForm.F_ClearData();
                ...
                if (FrmName == "用户设置")
                {
                    PerForm.F_User FrmUser = new 企业管理.PerForm.F_User();
                    FrmUser.Text = "用户设置";
                    FrmUser.ShowDialog();
                    FrmUser.Dispose();
                }
                ...
                { //使用 SaveFileDialog 让用户选择保存路径
					...
					saveFileDialog.Filter = "文本文件 (*.txt)|*.txt|所有文件 (*.*)|*.*"; // 文件类型过滤器
                ...
                if (FrmName == "系统帮助")
                {
					string filePath = @"C:\Users\Administrator\Desktop\9.30后余事\信息管理综合实验\帮助.txt";  // 替换为实际路径
					System.Diagnostics.Process.Start("notepad.exe", filePath);
					//System.Diagnostics.Process.Start("C:\\Users\\Administrator\\Desktop\\9.30后余事\\信息管理综合实验\\帮助.txt");
                }
			}
            if (n == 2)
            {
                ...
                InfoAddForm.F_Basic FrmBasic = new 企业管理.InfoAddForm.F_Basic();
        ...
        {
            cobox.Items.Clear();
            DataClass.MyMeans MyDataClsaa = new 企业管理.DataClass.MyMeans();
